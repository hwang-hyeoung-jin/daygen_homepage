            ...
            return BIT.callAjax('<c:url value="/frontbbs/getBbsList.do" />', 'post', param, function (response) {
                if (response.IsSucceed) {
                    if (response.totalrecords > 0) {
                        ...
                        $container.find('[name=BbsList]').empty().append("<div style='text-align: center; line-height: 100px; font-size: 26px;color: #3E3E3E;font-weight:500;padding:90px 0;'>뉴스가 없습니다.</div>");
                        $container.find('[name=btnMore]').hide();
                    }
                }
            });
