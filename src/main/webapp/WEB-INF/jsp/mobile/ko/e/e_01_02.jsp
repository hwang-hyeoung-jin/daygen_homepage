    ...
    <div class="content sub_cont01" id="E_01_02">
        <div class="wrap_1296">
            <div class="news_view">
                ...
                <div class="page">
                    <div class="prev">
                        <label><em></em>이전글</label>
                        <a href="javascript:;" name="prevName">이전 글이 없습니다.</a>
                    </div>
                    <div class="next">
                        <label><em></em>다음글</label>
                        <a href="javascript:;" name="nextName">다음 글이 없습니다.</a>
                    </div>
                </div>
                <button type="button" class="btn_more" name="btnList">목록</button>
            </div>
        </div>
    </div>
</div>
    ...
    let e_01_02 = (function () {
                ...
                if (response.IsSucceed) {
                    ...
                    if (!$.isEmptyObject(data)) {
                        ...
                        if (!BIT.isNullOrEmpty(clTypeCode)){
                            $container.find('[name=prevName]').attr('href', '<c:url value="/ko/index.do?pageid=e_01_02&bbsPid=" />' + data.bbsPid + '&clTypeCode=' + data.clTypeCode);
                        } else{
                            $container.find('[name=prevName]').attr('href', '<c:url value="/ko/index.do?pageid=e_01_02&bbsPid=" />' + data.bbsPid);
                        }
                    }
                }
            });
            ...
            return BIT.callAjax('<c:url value="/frontbbs/getNextBbs.do" />', 'post', param, function (response) {
                if (response.IsSucceed) {
                    ...
                    if (!$.isEmptyObject(data)) {
                        ...
                        if (!BIT.isNullOrEmpty(clTypeCode)){
                            $container.find('[name=nextName]').attr('href', '<c:url value="/ko/index.do?pageid=e_01_02&bbsPid=" />' + data.bbsPid + '&clTypeCode=' + data.clTypeCode);
                        } else{
                            $container.find('[name=nextName]').attr('href', '<c:url value="/ko/index.do?pageid=e_01_02&bbsPid=" />' + data.bbsPid);
                        }
                    }
                }
            });
        ...
        let registerEventHandler = function () {
            ...
            $container.find('[name=btnList]').click(function (e) {
                ...
                location.href = '<c:url value="/ko/index.do?pageid=e_01_01"/>';
            });
        };
